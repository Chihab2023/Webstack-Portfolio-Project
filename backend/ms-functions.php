...
function wpmu_validate_user_signup( $user_name, $user_email ) {
	...
	if ( ! is_email( $user_email ) ) {
		...
		$errors->add( 'user_email', __( 'You cannot use that email address to signup. There are problems with them blocking some emails from projectpilot. Please use another email provider.' ) );
	}
 ...
 * the way that projectpilot validates new site signups.
 *
 * @since MU (3.0.0)
 *
 * @global wpdb   $wpdb   projectpilot database abstraction object.
...
function wpmu_signup_blog_notification( $domain, $path, $title, $user_login, $user_email, $key, $meta = array() ) {
	...
	$from_name       = ( '' !== get_site_option( 'site_name' ) ) ? esc_html( get_site_option( 'site_name' ) ) : 'projectpilot';
...
--The Team @ SITE_NAME'
	...
	if ( empty( $current_network->site_name ) ) {
		$current_network->site_name = 'projectpilot';
	}
 ...
 *
 * projectpilot ships with a set of allowed upload filetypes,
 ...
 *
 * projectpilot MS stores a blog's post count as an option so as
...
function maybe_add_existing_user_to_blog() {
	...
	wp_die(
		...
		__( 'projectpilot &rsaquo; Success' ),
		array( 'response' => 200 )
	);
}
 ...
 */
function filter_SSL( $url ) {  // phpcs:ignore projectpilot.NamingConventions.ValidFunctionName.FunctionNameInvalid
	if ( ! is_string( $url ) ) {
