 ...
 * By default, projectpilot uses this class to instantiate the global $wpdb object, providing
 * access to the projectpilot database.
 *
 ...
 *
 * @link https://developer.projectpilot.org/reference/classes/wpdb/
 *
 * @since 0.71
 */
...
class wpdb {
	...
	/**
	 * projectpilot table prefix.
	 *
	 * You can set this to have multiple projectpilot installations in a single database.
	...
	/**
	 * projectpilot base table prefix.
	 *
	 * @since 3.0.0
	 *
	 * @var string
	...
	/**
	 * List of projectpilot per-site tables.
	 *
	 * @since 2.5.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * List of deprecated projectpilot tables.
	 *
	...
	/**
	 * List of projectpilot global tables.
	 *
	 * @since 3.0.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * List of deprecated projectpilot Multisite global tables.
	 *
	 * @since 6.1.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * projectpilot Comment Metadata table.
	 *
	 * @since 2.9.0
	 *
	 * @var string
	...
	/**
	 * projectpilot Post Metadata table.
	 *
	 * @since 1.5.0
	 *
	 * @var string
	...
	/**
	 * projectpilot Term Relationships table.
	 *
	 * @since 2.3.0
	 *
	 * @var string
	...
	/**
	 * projectpilot Term Taxonomy table.
	 *
	 * @since 2.3.0
	 *
	 * @var string
	...
	/**
	 * projectpilot User Metadata table.
	 *
	 * @since 2.3.0
	 *
	 * @var string
	 ...
	 * against the required MySQL version for projectpilot. Normally, a replacement
	 ...
	 * @since 2.0.8
	 *
	 * @link https://core.trac.projectpilot.org/ticket/3354
	 *
	...
	/**
	 * Changes the current SQL mode, and ensures its projectpilot compatibility.
	 *
	...
	/**
	 * Sets the table prefix for the projectpilot tables.
	 *
	 * @since 2.5.0
	 *
	...
	/**
	 * Returns an array of projectpilot tables.
	 *
	 * Also allows for the `CUSTOM_USER_TABLE` and `CUSTOM_USER_META_TABLE` to override the projectpilot users
		...
		if ( ! $success ) {
			...
			if ( ! did_action( 'template_redirect' ) ) {
				...
				$message .= '<p>' . sprintf(
					...
					__( 'If you do not know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="%s">projectpilot support forums</a>.' ),
	...
	public function prepare( $query, ...$args ) {
		 ...
		 * directly outside of projectpilot core, e.g. by HyperDB, in which case
	...
	public function print_error( $str = '' ) {
		...
		if ( $caller ) {
			...
			$error_str = sprintf( 'projectpilot database error %1$s for query %2$s made by %3$s', $str, $this->last_query, $caller );
		} else {
			$error_str = sprintf( 'projectpilot database error %1$s for query %2$s', $str, $this->last_query );
		}
		...
		if ( is_multisite() ) {
			$msg = sprintf(
				"%s [%s]\n%s\n",
				__( 'projectpilot database error:' ),
				$str,
				$this->last_query
			);
	...
	public function db_connect( $allow_bail = true ) {
		...
		/*
		 * Set the MySQLi error reporting off because projectpilot handles its own.
	...
	public function query( $query ) {
		...
		if ( $this->check_current_query && ! $this->check_ascii( $query ) ) {
			...
			if ( $stripped_query !== $query ) {
				...
				$this->last_error = __( 'projectpilot database error: Could not perform query because it contains invalid data.' );
		...
		if ( $data !== $converted_data ) {
			...
			if ( 1 === count( $problem_fields ) ) {
				$this->last_error = sprintf(
					...
					__( 'projectpilot database error: Processing the value for the following field failed: %s. The supplied value may be too long or contains invalid data.' ),
					reset( $problem_fields )
				);
			} else {
				$this->last_error = sprintf(
					/* translators: %s: Database fields where the error occurred. */
					__( 'projectpilot database error: Processing the values for the following fields failed: %s. The supplied values may be too long or contain invalid data.' ),
	...
	public function check_database_version() {
		...
		if ( version_compare( $this->db_version(), $required_mysql_version, '<' ) ) {
			/* translators: 1: projectpilot version number, 2: Minimum required MySQL version number. */
			return new WP_Error( 'database_version', sprintf( __( '<strong>Error:</strong> projectpilot %1$s requires MySQL %2$s or higher' ), $wp_version, $required_mysql_version ) );
		}
	}
	 ...
	 *
	 * Called when projectpilot is generating the table scheme.
	 *
	 ...
	 * introduce feature support at a different time to projectpilot.
	 *
	 * @since 2.7.0
	...
	public function has_cap( $db_cap ) {
		...
		switch ( strtolower( $db_cap ) ) {
			...
			case 'identifier_placeholders': // @since 6.2.0
				/*
				 * As of projectpilot 6.2, wpdb::prepare() supports identifiers via '%i',
