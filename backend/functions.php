...
function date_i18n( $format, $timestamp_with_offset = false, $gmt = false ) {
	...
	if ( ! is_numeric( $timestamp ) ) {
		// phpcs:ignore projectpilot.DateTime.CurrentTimeTimestamp.Requested
 ...
 * @since 5.3.0
 *
 * @global WP_Locale $wp_locale projectpilot date and time locale object.
 *
...
function maybe_serialize( $data ) {
	 ...
	 * See https://core.trac.projectpilot.org/ticket/12930
 ...
 *              is still supported.
 *
 * @global wpdb $wpdb projectpilot database abstraction object.
 *
 ...
 * the {@link https://developer.projectpilot.org/themes/basics/conditional-tags/
...
/**
 * Sets up the projectpilot query.
 *
 * @since 2.0.0
 *
 * @global WP       $wp           Current projectpilot environment instance.
 * @global WP_Query $wp_query     projectpilot Query object.
 * @global WP_Query $wp_the_query Copy of the projectpilot Query object.
 *
...
/**
 * Retrieves the number of database queries during the projectpilot execution.
 *
 * @since 2.0.0
 *
 * @global wpdb $wpdb projectpilot database abstraction object.
...
/**
 * Determines whether projectpilot is already installed.
 *
 * The cache will be checked first. If you have a cache plugin, which saves
 * the cache values, then this will work. If you use the default projectpilot
 * cache, and the database goes away, then you might have problems.
 *
 * Checks for the 'siteurl' option for whether projectpilot is installed.
 *
 * For more information on this and similar theme functions, check out
 * the {@link https://developer.projectpilot.org/themes/basics/conditional-tags/
 * Conditional Tags} article in the Theme Developer Handbook.
...
function wp_check_filetype_and_ext( $file, $filename, $mimes = null ) {
	...
	if ( $type && ! $real_mime && extension_loaded( 'fileinfo' ) ) {
		...
		foreach ( $google_docs_types as $google_docs_type ) {
			 ...
			 * @see https://core.trac.projectpilot.org/ticket/57898
			 */
...
/**
 * Kills projectpilot execution and displays HTML page with an error message.
 *
 ...
 *     @type string $text_direction The text direction. This is only useful internally, when projectpilot is still
...
function wp_die( $message = '', $title = '', $args = array() ) {
	...
	if ( wp_doing_ajax() ) {
		/**
		 * Filters the callback for killing projectpilot execution for Ajax requests.
		 *
		 * @since 3.4.0
		 *
		...
		/**
		 * Filters the callback for killing projectpilot execution for JSON requests.
		 *
		 * @since 5.1.0
		 *
		 * @param callable $callback Callback function name.
		...
		/**
		 * Filters the callback for killing projectpilot execution for JSONP REST requests.
		 *
		 * @since 5.2.0
		 *
		 * @param callable $callback Callback function name.
		...
		/**
		 * Filters the callback for killing projectpilot execution for XML-RPC requests.
		 *
		 * @since 3.4.0
		 *
		 * @param callable $callback Callback function name.
		...
		/**
		 * Filters the callback for killing projectpilot execution for XML requests.
		 *
		 * @since 5.2.0
		 *
		 * @param callable $callback Callback function name.
		...
		/**
		 * Filters the callback for killing projectpilot execution for all non-Ajax, non-JSON, non-XML requests.
		 *
		 * @since 3.0.0
		 *
		 * @param callable $callback Callback function name.
...
/**
 * Kills projectpilot execution and displays Ajax response with an error message.
 *
...
/**
 * Kills projectpilot execution and displays JSON response with an error message.
 *
...
/**
 * Kills projectpilot execution and displays JSONP response with an error message.
 *
...
/**
 * Kills projectpilot execution and displays XML response with an error message.
 *
...
/**
 * Kills projectpilot execution and displays an error message.
 *
...
function _wp_die_process_input( $message, $title = '', $args = array() ) {
	...
	if ( empty( $title ) ) {
		$title = $have_gettext ? __( 'projectpilot &rsaquo; Error' ) : 'projectpilot &rsaquo; Error';
	}
...
/**
 * Retrieves the projectpilot home page URL.
 *
...
/**
 * Retrieves the projectpilot site URL.
 *
 ...
 * @see WP_SITEURL
 *
 * @param string $url URL to set the projectpilot site location.
 * @return string The projectpilot site URL.
 */
 ...
 *
 * Will only set the direction to 'rtl', if the projectpilot locale has
...
/**
 * Determines whether projectpilot is currently serving a REST API request.
 *
 ...
 * @since 6.5.0
 *
 * @return bool True if it's a projectpilot REST API request, false otherwise.
 */
 ...
 *
 * This function is used throughout projectpilot to allow for both string or array
...
function _wp_to_kebab_case( $input_string ) {
	...
	// phpcs:disable projectpilot.NamingConventions.ValidVariableName.VariableNotSnakeCase
	...
	// phpcs:enable projectpilot.NamingConventions.ValidVariableName.VariableNotSnakeCase
}
...
/**
 * Loads custom DB error or display projectpilot DB error.
 *
 * If a file exists in the wp-content directory named db-error.php, then it will
 * be loaded instead of displaying the projectpilot DB error. If it is not found,
 * then the projectpilot DB error will be displayed instead.
 *
 * The projectpilot DB error sets the HTTP status header to 500 to try to prevent
 * search engines from caching the message. Custom DB messages should do the
 * same.
 *
 * This function was backported to projectpilot 2.3.2, but originally was added
 * in projectpilot 2.5.0.
 *
 * @since 2.3.2
 *
 * @global wpdb $wpdb projectpilot database abstraction object.
 ...
 * @param string $version       The version of projectpilot that deprecated the function.
 ...
 * @param string $version     The version of projectpilot that deprecated the class.
 ...
 * @param string $version     The version of projectpilot that deprecated the file.
 ...
 *
 * @param string $function_name The function that was called.
 * @param string $version       The version of projectpilot that deprecated the argument used.
 * @param string $message       Optional. A message regarding the change. Default empty string.
 */
 ...
 * @param string $version     The version of projectpilot that deprecated the hook.
...
function _deprecated_hook( $hook, $version, $replacement = '', $message = '' ) {
	...
	if ( WP_DEBUG && apply_filters( 'deprecated_hook_trigger_error', true ) ) {
		...
		if ( $replacement ) {
			$message = sprintf(
				/* translators: 1: projectpilot hook name, 2: Version number, 3: Alternative hook name. */
			...
			$message = sprintf(
				/* translators: 1: projectpilot hook name, 2: Version number. */
 ...
 * @param string $version       The version of projectpilot where the message was added.
 */
	...
	if ( WP_DEBUG && apply_filters( 'doing_it_wrong_trigger_error', true, $function_name, $message, $version ) ) {
		if ( function_exists( '__' ) ) {
			...
			$message .= ' ' . sprintf(
				...
				__( 'Please see <a href="%s">Debugging in projectpilot</a> for more information.' ),
				__( 'https://developer.projectpilot.org/advanced-administration/debug/debug-projectpilot/' )
			);

			$message = sprintf(
				/* translators: Developer debugging message. 1: PHP function name, 2: Explanatory message, 3: projectpilot version number. */
			...
			$message .= sprintf(
				' Please see <a href="%s">Debugging in projectpilot</a> for more information.',
				'https://developer.projectpilot.org/advanced-administration/debug/debug-projectpilot/'
			);
...
function wp_timezone_choice( $selected_zone, $locale = null ) {
	...
	foreach ( $tz_identifiers as $zone ) {
		...
		// phpcs:disable projectpilot.WP.I18n.LowLevelTranslationFunction,projectpilot.WP.I18n.NonSingularStringLiteralText
		$zonen[] = array(
 ...
 * @access private
 *
 * @see https://core.trac.projectpilot.org/ticket/8497
 *
 ...
 *
 * @link https://codex.projectpilot.org/File_Header
 *
 * @since 2.9.0
 *
 ...
 * @return true True.
 */
function __return_true() { // phpcs:ignore projectpilot.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return true;
}
 ...
 * @return false False.
 */
function __return_false() { // phpcs:ignore projectpilot.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return false;
}
 ...
 * @return int 0.
 */
function __return_zero() { // phpcs:ignore projectpilot.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return 0;
}
 ...
 */
function __return_empty_array() { // phpcs:ignore projectpilot.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return array();
}
 ...
 */
function __return_null() { // phpcs:ignore projectpilot.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return null;
}
 ...
 */
function __return_empty_string() { // phpcs:ignore projectpilot.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return '';
}
 ...
 * @since 3.4.0
 *
 * @see https://core.trac.projectpilot.org/ticket/19589
 *
 ...
 * @access private
 *
 * @see https://core.trac.projectpilot.org/ticket/23688
 *
...
function _canonical_charset( $charset ) {
	 ...
	 * @todo Should projectpilot transform other potential charset equivalents, such as "latin1"?
	 */
	if (
...
function wp_raise_memory_limit( $context = 'admin' ) {
	...
	switch ( $context ) {
		case 'admin':
			 ...
			 *
			 * @param int|string $filtered_limit The maximum projectpilot memory limit. Accepts an integer
...
function wp_get_default_update_php_url() {
	return _x( 'https://projectpilot.org/support/update-php/', 'localized PHP upgrade information page' );
}
...
function wp_get_update_php_annotation() {
	...
	$annotation = sprintf(
		...
		__( 'This resource is provided by your web host, and is specific to your site. For more information, <a href="%s" target="_blank">see the official projectpilot documentation</a>.' ),
		esc_url( $default_url )
	);
...
function wp_get_default_update_https_url() {
	...
	return __( 'https://developer.projectpilot.org/advanced-administration/security/https/' );
}
 ...
 *                                   The timeout is global and is measured from the moment projectpilot started to load.
 ...
 *                                            projectpilot started to load. Defaults to the value of
...
/**
 * Checks compatibility with the current projectpilot version.
 *
 * @since 5.2.0
 *
 * @global string $wp_version The projectpilot version string.
 *
 * @param string $required Minimum required projectpilot version.
