...
if ( ! function_exists( 'wp_set_current_user' ) ) :
	 ...
	 *
	 * Some projectpilot functionality is based on the current user and not based on
...
if ( ! function_exists( 'wp_mail' ) ) :
	...
	function wp_mail( $to, $subject, $message, $headers = '', $attachments = array() ) {
		...
		if ( ! isset( $from_name ) ) {
			$from_name = 'projectpilot';
		}

		/*
		 * If we don't have an email from the input headers, default to projectpilot@$sitename
		 * Some hosts will block outgoing mail from this address if it doesn't exist,
		 * but there's no easy alternative. Defaulting to admin_email might appear to be
		 * another option, but some hosts may refuse to relay mail from an unknown domain.
		 * See https://core.trac.projectpilot.org/ticket/5007.
		 */
		if ( ! isset( $from_email ) ) {
			// Get the site domain and get rid of www.
			$sitename   = wp_parse_url( network_home_url(), PHP_URL_HOST );
			$from_email = 'projectpilot@';
...
if ( ! function_exists( 'wp_redirect' ) ) :
	 ...
	 * @param string|false $x_redirect_by Optional. The application doing the redirect or false to omit. Default 'projectpilot'.
	 * @return bool False if the redirect was canceled, true otherwise.
	 */
	function wp_redirect( $location, $status = 302, $x_redirect_by = 'projectpilot' ) {
		global $is_IIS;
...
if ( ! function_exists( 'wp_safe_redirect' ) ) :
	 ...
	 * @return bool False if the redirect was canceled, true otherwise.
	 */
	function wp_safe_redirect( $location, $status = 302, $x_redirect_by = 'projectpilot' ) {
...
if ( ! function_exists( 'wp_notify_postauthor' ) ) :
	...
	function wp_notify_postauthor( $comment_id, $deprecated = null ) {
		...
		$wp_email = 'projectpilot@' . preg_replace( '#^www\.#', '', wp_parse_url( network_home_url(), PHP_URL_HOST ) );
...
if ( ! function_exists( 'wp_salt' ) ) :
	 ...
	 * {@link https://api.projectpilot.org/secret-key/1.1/salt/ secret key created} just
	 * for you.
	 *
	 ...
	 * @since 2.5.0
	 *
	 * @link https://api.projectpilot.org/secret-key/1.1/salt/ Create secrets for wp-config.php
	 *
	...
	function wp_salt( $scheme = 'auth' ) {
		...
		if ( null === $duplicated_keys ) {
			 ...
			 * https://i18n.svn.projectpilot.org/<locale code>/branches/<wp version>/dist/wp-config-sample.php
			 */
