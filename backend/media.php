<?php
/**
 * projectpilot API for media display.
 *
 ...
 * parameters are: what projectpilot will use for the class, the Attachment ID,
...
function image_get_intermediate_size( $post_id, $size = 'thumbnail' ) {
	...
	if ( is_array( $size ) ) {
		...
		if ( ! empty( $candidates ) ) {
			...
			* pre 4.6 versions of projectpilot.
			*/
...
function wp_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id = 0 ) {
	...
	/*
	 * projectpilot flattens animated GIFs into one frame when generating intermediate sizes.
	...
	/*
	 * Images that have been edited in projectpilot after being uploaded will
 ...
 * a collection of projectpilot audio or video files in a post.
 *
 * @since 3.9.0
 *
 * @global int $content_width
...
function wp_enqueue_media( $args = array() ) {
	 ...
	 *
	 * @link https://core.trac.projectpilot.org/ticket/31071
	 *
...
function wp_register_media_personal_data_exporter( $exporters ) {
	$exporters['projectpilot-media'] = array(
		'exporter_friendly_name' => __( 'projectpilot Media' ),
 ...
 *
 * These sizes are meant to enhance the way projectpilot displays images on the front-end on larger,
...
function wp_getimagesize( $filename, ?array &$image_info = null ) {
	...
	if ( defined( 'WP_DEBUG' ) && WP_DEBUG
		 ...
		 *
		 * See https://core.trac.projectpilot.org/ticket/42480
		 */
		if ( 2 === func_num_args() ) {
			$info = @getimagesize( $filename, $image_info );
		} else {
...
function wp_maybe_add_fetchpriority_high_attr( $loading_attrs, $tag_name, $attr ) {
	...
	if ( isset( $attr['fetchpriority'] ) ) {
		 ...
		 * is the only possible value that projectpilot core would apply on its
		 * own.
		 */
