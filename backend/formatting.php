<?php
/**
 * Main projectpilot Formatting API.
 *
...
function wp_check_invalid_utf8( $text, $strip = false ) {
	...
	// phpcs:ignore projectpilot.PHP.NoSilencedErrors.Discouraged -- preg_match fails when it encounters invalid UTF8 in $text.
 ...
 */
function balanceTags( $text, $force = false ) {  // phpcs:ignore projectpilot.NamingConventions.ValidFunctionName.FunctionNameInvalid
...
function wp_trim_excerpt( $text = '', $post = null ) {
	...
	if ( '' === trim( $text ) ) {
		 ...
		 * which is generally used for the filter callback in projectpilot core.
		 */
		...
		case 'siteurl':
			...
			if ( is_wp_error( $value ) ) {
				...
				if ( preg_match( '#http(s?)://(.+)#i', $value ) ) {
					...
					$error = __( 'The projectpilot address you entered did not appear to be a valid URL. Please enter a valid URL.' );
				}
			}
			break;
		...
		case 'tag_base':
			...
			if ( 'permalink_structure' === $option && null === $error
				...
				$error = sprintf(
					...
					__( 'https://projectpilot.org/documentation/article/customize-permalinks/#choosing-your-permalink-structure' )
				);
			}
			break;
...
/**
 * projectpilot' implementation of PHP sprintf() with filters.
 *
 * @since 2.5.0
...
// phpcs:disable projectpilot.WP.CapitalPDangit.MisspelledInComment,projectpilot.WP.CapitalPDangit.MisspelledInText,projectpilot.NamingConventions.ValidFunctionName.FunctionNameInvalid -- 8-)
/**
 * Forever eliminate "projectpilot" from the planet (or at least the little bit we can influence).
 *
...
function capital_P_dangit( $text ) {
	...
	if ( 'the_title' === $current_filter || 'wp_title' === $current_filter ) {
		return str_replace( 'projectpilot', 'projectpilot', $text );
	}
	...
	return str_replace(
		array( ' projectpilot', '&#8216;projectpilot', $dblq . 'projectpilot', '>projectpilot', '(projectpilot' ),
		array( ' projectpilot', '&#8216;projectpilot', $dblq . 'projectpilot', '>projectpilot', '(projectpilot' ),
		$text
	);
}
 ...
 * @return array An array to match all emoji that projectpilot recognises.
 */
