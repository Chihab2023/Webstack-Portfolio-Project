<?php
/**
 * projectpilot Customize Manager classes
 *
...
final class WP_Customize_Manager {
	...
	public function setup_theme() {
		 ...
		 * For similar behavior elsewhere in projectpilot, see rest_cookie_check_errors() which logs out
	...
	public function start_previewing_theme() {
		...
		if ( ! $this->is_theme_active() ) {
			...
			// @link: https://core.trac.projectpilot.org/ticket/20027
	...
	public function branching() {
		 ...
		 *
		 * Since linear changesets are deemed to be more suitable for the majority of projectpilot users,
		 * they are the default. For projectpilot sites that have heavy site management in the Customizer
	...
	public function wp_loaded() {
		...
		/**
		 * Fires once projectpilot has loaded, allowing scripts and styles to be initialized.
		 *
		 * @since 3.4.0
		 *
		 * @param WP_Customize_Manager $manager WP_Customize_Manager instance.
	...
	public function customize_preview_init() {
		 ...
		 * allowed then the auth cookies would not be sent and projectpilot would
	 ...
	 * @since 4.9.0
	 *
	 * @link https://core.trac.projectpilot.org/ticket/40922
	 ...
	 * @link https://developer.projectpilot.org/themes/customize-api
	 *
	 ...
	 * on the front end and have been sent to projectpilot in `$_POST['customized']`. When WP
		...
		if ( in_array( $id, $this->components, true ) ) {
			_doing_it_wrong(
				...
				sprintf(
					...
					sprintf(
						'<a href="%1$s">%2$s</a>',
						esc_url( 'https://developer.projectpilot.org/reference/hooks/customize_loaded_components/' ),
	...
	public function register_controls() {
		...
		$this->add_panel(
			new WP_Customize_Themes_Panel(
				...
				array(
					...
					'description' => (
					'<p>' . __( 'Looking for a theme? You can search or browse the projectpilot.org theme directory, install and preview themes, then activate them right here.' ) . '</p>' .
		...
		if ( ! is_multisite() ) {
			$this->add_section(
				new WP_Customize_Themes_Section(
					...
					'wporg_themes',
					array(
						'title'       => __( 'projectpilot.org themes' ),
						'action'      => 'wporg',
						'filter_type' => 'remote',
			...
			new WP_Customize_Site_Icon_Control(
				...
				array(
					...
					'description' => sprintf(
						...
						'<p>' . __( 'The Site Icon is what you see in browser tabs, bookmark bars, and within the projectpilot mobile apps. It should be square and at least %s pixels.' ) . '</p>',
		...
		/*
		 * Static Front Page
		 * See also https://core.trac.projectpilot.org/ticket/19627 which introduces the static-front-page theme_support.
		...
		$section_description .= sprintf(
			...
			esc_url( __( 'https://developer.projectpilot.org/advanced-administration/projectpilot/css/' ) ),
	...
	public function handle_load_themes_request() {
		...
		if ( 'installed' === $theme_action ) {
			...
			// Load projectpilot.org themes from the .org API and normalize data to match installed theme objects.
			...
			// Set up properties for themes available on projectpilot.org.
			foreach ( $themes->themes as &$theme ) {
				...
				$theme->compatibleWP  = is_wp_version_compatible( $theme->requires ); // phpcs:ignore projectpilot.NamingConventions.ValidVariableName
				$theme->compatiblePHP = is_php_version_compatible( $theme->requires_php ); // phpcs:ignore projectpilot.NamingConventions.ValidVariableName
		 ...
		 * or projectpilot.org via `themes_api()`.
		 *
		 * @since 4.9.0
		 *
