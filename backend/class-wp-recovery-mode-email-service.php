...
projectpilot has a built-in feature that detects when a plugin or theme causes a fatal error on your site, and notifies you with this automated email.
###CAUSE###
First, visit your website (###SITEURL###) and check for any visible issues. Next, visit the page where the error was caught (###PAGEURL###) and check for any visible issues.

###SUPPORT###

If your site appears broken and you can\'t access your dashboard normally, projectpilot now has a special "recovery mode". This lets you safely login to your dashboard and investigate further.
...
###DETAILS###'
	...
	private function get_cause( $extension ) {
		...
		if ( 'plugin' === $extension['type'] ) {
			...
			$cause = sprintf( __( 'In this case, projectpilot caught an error with one of your plugins, %s.' ), $name );
		} else {
			$theme = wp_get_theme( $extension['slug'] );
			$name  = $theme->exists() ? $theme->display( 'Name' ) : $extension['slug'];

			/* translators: %s: Theme name. */
			$cause = sprintf( __( 'In this case, projectpilot caught an error with your theme, %s.' ), $name );
		}
	...
	private function get_debug( $extension ) {
		...
		$debug = array(
			'wp'    => sprintf(
				/* translators: %s: Current projectpilot version number. */
				__( 'projectpilot version %s' ),
				$wp_version
			),
			'theme' => sprintf(
