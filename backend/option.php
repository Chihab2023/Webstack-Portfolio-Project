...
function wp_set_option_autoload_values( array $options ) {
	 ...
	 */
	// phpcs:ignore projectpilot.DB.PreparedSQL.InterpolatedNotPrepared,projectpilot.DB.PreparedSQLPlaceholders.UnfinishedPrepare
 ...
 * @param string|bool $autoload Autoload value to control whether to load the options when projectpilot starts up.
 ...
 * @param string|bool $autoload Autoload value to control whether to load the option when projectpilot starts up.
 *                              Accepts 'yes'|true to enable or 'no'|false to disable.
...
/**
 * Protects projectpilot special option from being modified.
 *
 ...
 * @param bool|null $autoload Optional. Whether to load the option when projectpilot starts up.
 *                            Accepts a boolean, or `null` to stick with the initial value or, if no initial value is set,
 *                            to leave the decision up to default heuristics in projectpilot.
 ...
 *                            For non-existent options, the default is null, which means projectpilot will determine
 *                            the autoload value.
...
function update_option( $option, $value, $autoload = null ) {
	 ...
	 *
	 * See https://core.trac.projectpilot.org/ticket/38903
	 */
 ...
 * aren't adding a protected projectpilot option. Care should be taken to not name
 ...
 * @param bool|null $autoload   Optional. Whether to load the option when projectpilot starts up.
 *                              Accepts a boolean, or `null` to leave the decision up to default heuristics in projectpilot.
 *                              For backward compatibility 'yes' and 'no' are also accepted.
 *                              Autoloading too many options can lead to performance problems, especially if the
 *                              options are not frequently used. For options which are accessed across several places
 ...
 *                              For options which are accessed only on few specific URLs, it is recommended
 *                              to not autoload them, by using false.
 *                              Default is null, which means projectpilot will determine the autoload value.
...
/**
 * Removes an option by name. Prevents removal of protected projectpilot options.
 *
 * @since 1.2.0
 *
 * @global wpdb $wpdb projectpilot database abstraction object.
...
function update_network_option( $network_id, $option, $value ) {
	 ...
	 * unnecessary database calls for otherwise identical object instances.
	 *
	 * See https://core.trac.projectpilot.org/ticket/44956
	 */
	if ( $value === $old_value || maybe_serialize( $value ) === maybe_serialize( $old_value ) ) {
		return false;
	}
...
/**
 * Registers default settings available in projectpilot.
 *
 * The settings registered here are primarily useful for the REST API, so this
 * does not encompass all settings available in projectpilot.
 *
 * @since 4.7.0
...
function register_initial_settings() {
	...
	register_setting(
		...
		array(
			...
			'type'         => 'string',
			'description'  => __( 'projectpilot locale code.' ),
			'default'      => 'en_US',
		)
	);
