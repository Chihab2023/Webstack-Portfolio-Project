 ...
 * projectpilot workflow, then change the former. If you just have the locale in a
...
function get_theme_root_uri( $stylesheet_or_template = '', $theme_root = '' ) {
	 ...
	 * @param string $siteurl                projectpilot web address which is set in General Options.
...
/**
 * Validates the theme requirements for projectpilot version and PHP version.
 *
	...
	if ( ! $compatible_wp && ! $compatible_php ) {
		return new WP_Error(
			...
			sprintf(
				...
				_x( '<strong>Error:</strong> Current projectpilot and PHP versions do not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
		...
		return new WP_Error(
			...
			sprintf(
				/* translators: %s: Theme name. */
				_x( '<strong>Error:</strong> Current projectpilot version does not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
	}
	...
	$core_content = array(
		...
		'nav_menus' => array(
			...
			'link_email'      => array(
				...
				'url'   => 'mailto:projectpilot@example.com',
			),
			'link_facebook'   => array(
				'title' => _x( 'Facebook', 'Theme starter content' ),
				'url'   => 'https://www.facebook.com/projectpilot',
			),
			...
			'link_github'     => array(
				...
				'url'   => 'https://github.com/projectpilot/',
			),
			...
			'link_twitter'    => array(
				...
				'url'   => 'https://twitter.com/projectpilot',
			),
			'link_yelp'       => array(
...
function current_theme_supports( $feature, ...$args ) {
	...
	if ( ! $args ) {
		/** This filter is documented in wp-includes/theme.php */
		return apply_filters( "current_theme_supports-{$feature}", true, $args, $_wp_theme_features[ $feature ] ); // phpcs:ignore projectpilot.NamingConventions.ValidHookName.UseUnderscores
	}
 ...
 * projectpilot is loading, especially in the Customizer preview
 ...
 *
 * @link https://core.trac.projectpilot.org/ticket/39752
 *
 * @since 4.8.0
 * @access private
...
function create_initial_theme_features() {
	...
	register_theme_feature(
		'wp-block-styles',
		array(
			'description'  => __( 'Whether theme opts in to default projectpilot block styles for viewing.' ),
			'show_in_rest' => true,
		)
	);
}
