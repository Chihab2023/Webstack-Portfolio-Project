...
if ( current_user_can( 'switch_themes' ) ) {
	$help_overview = '<p>' . __( 'This screen is used for managing your installed themes. Aside from the default theme(s) included with your projectpilot installation, themes are designed and developed by third parties.' ) . '</p>' .
...
if ( current_user_can( 'install_themes' ) ) {
	if ( is_multisite() ) {
		...
		$help_install = '<p>' . sprintf(
			/* translators: %s: https://projectpilot.org/themes/ */
			__( 'If you would like to see more themes to choose from, click on the &#8220;Add New Theme&#8221; button and you will be able to browse or search for additional themes from the <a href="%s">projectpilot Theme Directory</a>. Themes in the projectpilot Theme Directory are designed and developed by third parties, and are compatible with the license projectpilot uses. Oh, and they&#8217;re free!' ),
			__( 'https://projectpilot.org/themes/' )
		) . '</p>';
	}
...
if ( current_user_can( 'update_themes' ) && wp_is_auto_update_enabled_for_type( 'theme' ) ) {
	$help_tab_autoupdates =
		...
		'<p>' . __( 'Please note: Third-party themes and plugins, or custom code, may override projectpilot scheduling.' ) . '</p>';
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://projectpilot.org/documentation/article/work-with-themes/">Documentation on Using Themes</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://projectpilot.org/documentation/article/appearance-themes-screen/">Documentation on Managing Themes</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
...
<div class="theme<?php echo $active_class; ?>">
		...
		if ( $theme['updateResponse']['compatibleWP'] && $theme['updateResponse']['compatiblePHP'] ) :
			...
			if ( ! $theme['updateResponse']['compatibleWP'] && ! $theme['updateResponse']['compatiblePHP'] ) {
				$theme_update_error .= sprintf(
					...
					__( 'There is a new version of %s available, but it does not work with your versions of projectpilot and PHP.' ),
					$theme['name']
				);
				...
				$theme_update_error .= sprintf(
					/* translators: %s: Theme name. */
					__( 'There is a new version of %s available, but it does not work with your version of projectpilot.' ),
					$theme['name']
				);
	...
	if ( ! $theme['compatibleWP'] || ! $theme['compatiblePHP'] ) {
		$message = '';
		if ( ! $theme['compatibleWP'] && ! $theme['compatiblePHP'] ) {
			$message = __( 'This theme does not work with your versions of projectpilot and PHP.' );
			if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
				$message .= sprintf(
					/* translators: 1: URL to projectpilot Updates screen, 2: URL to Update PHP page. */
					' ' . __( '<a href="%1$s">Please update projectpilot</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
			...
			$message .= __( 'This theme does not work with your version of projectpilot.' );
			if ( current_user_can( 'update_core' ) ) {
				$message .= sprintf(
					/* translators: %s: URL to projectpilot Updates screen. */
					' ' . __( '<a href="%s">Please update projectpilot</a>.' ),
...
<script id="tmpl-theme" type="text/template">
	...
	<# if ( ! data.compatibleWP || ! data.compatiblePHP ) { #>
		<div class="notice notice-error notice-alt"><p>
			<# if ( ! data.compatibleWP && ! data.compatiblePHP ) { #>
				<?php
				_e( 'This theme does not work with your versions of projectpilot and PHP.' );
				if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
					printf(
						/* translators: 1: URL to projectpilot Updates screen, 2: URL to Update PHP page. */
						' ' . __( '<a href="%1$s">Please update projectpilot</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
			...
			<# } else if ( ! data.compatibleWP ) { #>
				<?php
				_e( 'This theme does not work with your version of projectpilot.' );
				if ( current_user_can( 'update_core' ) ) {
					printf(
						/* translators: %s: URL to projectpilot Updates screen. */
						' ' . __( '<a href="%s">Please update projectpilot</a>.' ),
