...
/** Load projectpilot Translation Installation API */
...
if ( file_exists( ABSPATH . 'wp-config-sample.php' ) ) {
	...
	wp_die(
		sprintf(
			...
			__( 'Sorry, I need a %s file to work from. Please re-upload this file to your projectpilot installation.' ),
...
if ( @file_exists( ABSPATH . '../wp-config.php' ) && ! @file_exists( ABSPATH . '../wp-settings.php' ) ) {
	wp_die(
		'<p>' . sprintf(
			/* translators: 1: wp-config.php, 2: install.php */
			__( 'The file %1$s already exists one level above your projectpilot installation. If you need to reset any of the configuration items in this file, please delete it first. You may try <a href="%2$s">installing now</a>.' ),
			'<code>wp-config.php</code>',
			'install.php'
		) . '</p>',
		409
...
<head>
	...
	<title><?php _e( 'projectpilot &rsaquo; Setup Configuration File' ); ?></title>
...
<p><?php _e( 'Welcome to projectpilot. Before getting started, you will need to know the following items.' ); ?></p>
<ol>
	<li><?php _e( 'Database name' ); ?></li>
	<li><?php _e( 'Database username' ); ?></li>
	<li><?php _e( 'Database password' ); ?></li>
	<li><?php _e( 'Database host' ); ?></li>
	<li><?php _e( 'Table prefix (if you want to run more than one projectpilot in a single database)' ); ?></li>
</ol>
<p>
		<?php
		printf(
	...
	<strong>
		...
		printf(
			...
			__( 'https://developer.projectpilot.org/advanced-administration/projectpilot/wp-config/' ),
			'<code>wp-config.php</code>'
		);
		?>
</p>
...
<form method="post" action="setup-config.php?step=2">
	...
	<table class="form-table" role="presentation">
		<tr>
			...
			<td><input name="dbname" id="dbname" type="text" aria-describedby="dbname-desc" size="25" placeholder="projectpilot"<?php echo $autofocus; ?>/>
			<p id="dbname-desc"><?php _e( 'The name of the database you want to use with projectpilot.' ); ?></p></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<p id="prefix-desc"><?php _e( 'If you want to run multiple projectpilot installations in a single database, change this.' ); ?></p></td>
		</tr>
	</table>
		<?php
		if ( isset( $_GET['noapi'] ) ) {
	...
	case 2:
		...
		*/
		...
		try {
			...
			if ( ! $no_api ) {
				$secret_keys = wp_remote_get( 'https://api.projectpilot.org/secret-key/1.1/salt/' );
			}
			...
			if ( false !== $handle ) {
				...
				if ( ! empty( $wp_config_perms ) && ! is_writable( $path_to_wp_config ) ) {
					$error_message = sprintf(
						...
						'<code>wp-config.php</code>',
						__( 'https://developer.projectpilot.org/advanced-administration/server/file-permissions/' )
					);
				} else {
					$error_message = sprintf(
						/* translators: %s: wp-config.php */
...
<p><?php _e( 'All right, sparky! You&#8217;ve made it through this part of the installation. projectpilot can now communicate with your database. If you are ready, time now to&hellip;' ); ?></p>
