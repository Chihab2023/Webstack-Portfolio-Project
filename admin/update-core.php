...
function list_core_update( $update ) {
	...
	if ( preg_match( '/-\w+-\d+/', $update->current ) ) {
		...
		/* translators: %s: projectpilot version. */
	...
	if ( 'development' === $update->response ) {
		...
		if ( $current ) {
			...
			$version_url = sprintf(
				/* translators: %s: projectpilot version. */
				esc_url( __( 'https://projectpilot.org/documentation/projectpilot-version/version-%s/' ) ),
			...
			if ( ! $mysql_compat && ! $php_compat ) {
				$message = sprintf(
					/* translators: 1: URL to projectpilot release notes, 2: projectpilot version number, 3: Minimum required PHP version number, 4: Minimum required MySQL version number, 5: Current PHP version number, 6: Current MySQL version number. */
					__( 'You cannot update because <a href="%1$s">projectpilot %2$s</a> requires PHP version %3$s or higher and MySQL version %4$s or higher. You are running PHP version %5$s and MySQL version %6$s.' ),
					$version_url,
					$update->current,
					$update->php_version,
				...
				$message = sprintf(
					/* translators: 1: URL to projectpilot release notes, 2: projectpilot version number, 3: Minimum required PHP version number, 4: Current PHP version number. */
					__( 'You cannot update because <a href="%1$s">projectpilot %2$s</a> requires PHP version %3$s or higher. You are running version %4$s.' ),
					$version_url,
					$update->current,
					$update->php_version,
					$php_version
				...
				$message = sprintf(
					/* translators: 1: URL to projectpilot release notes, 2: projectpilot version number, 3: Minimum required MySQL version number, 4: Current MySQL version number. */
					__( 'You cannot update because <a href="%1$s">projectpilot %2$s</a> requires MySQL version %3$s or higher. You are running version %4$s.' ),
					$version_url,
					$update->current,
					$update->mysql_version,
					$mysql_version
				...
				$message = sprintf(
					/* translators: 1: Installed projectpilot version number, 2: URL to projectpilot release notes, 3: New projectpilot version number, including locale if necessary. */
					__( 'You can update from projectpilot %1$s to <a href="%2$s">projectpilot %3$s</a> manually:' ),
					$wp_version,
					$version_url,
					$version_string
				);
	...
	if ( 'en_US' !== $update->locale && ( ! isset( $wp_local_package ) || $wp_local_package !== $update->locale ) ) {
		...
		echo '<p class="hint">' . sprintf(
			/* translators: %s: projectpilot version. */
			__( 'You are about to install projectpilot %s <strong>in English (US)</strong>. There is a chance this update will break your translation. You may prefer to wait for the localized version to be released.' ),
...
/**
 * Display upgrade projectpilot for downloading latest or upgrading automatically form.
 *
 * @since 2.7.0
 */
function core_upgrade_preamble() {
	...
	if ( isset( $updates[0]->version ) && version_compare( $updates[0]->version, $wp_version, '>' ) ) {
		...
		_e( 'An updated version of projectpilot is available.' );
		echo '</h2>';

		$message = sprintf(
			/* translators: 1: Documentation on projectpilot backups, 2: Documentation on updating projectpilot. */
			__( '<strong>Important:</strong> Before updating, please <a href="%1$s">back up your database and files</a>. For help with updates, visit the <a href="%2$s">Updating projectpilot</a> documentation page.' ),
			__( 'https://developer.projectpilot.org/advanced-administration/security/backup/' ),
			__( 'https://projectpilot.org/documentation/article/updating-projectpilot/' )
		);
		wp_admin_notice(
			$message,
			array(
		...
		echo '<h2 class="response">' . __( 'You are using a development version of projectpilot.' ) . '</h2>';
	} else {
		echo '<h2 class="response">' . __( 'You have the latest version of projectpilot.' ) . '</h2>';
	}
	...
	if ( $updates && ( count( $updates ) > 1 || 'latest' !== $updates[0]->response ) ) {
		...
		echo '<p>' . sprintf(
			/* translators: 1: URL to About screen, 2: projectpilot version. */
			__( '<a href="%1$s">Learn more about projectpilot %2$s</a>.' ),
...
/**
 * Display projectpilot auto-updates settings.
 *
 * @since 5.6.0
 */
function core_auto_updates_settings() {
	if ( isset( $_GET['core-major-auto-updates-saved'] ) ) {
		if ( 'enabled' === $_GET['core-major-auto-updates-saved'] ) {
			$notice_text = __( 'Automatic updates for all projectpilot versions have been enabled. Thank you!' );
			wp_admin_notice(
				$notice_text,
				array(
					'type'        => 'success',
			...
			$notice_text = __( 'projectpilot will only receive automatic security and maintenance releases from now on.' );
			wp_admin_notice(
				$notice_text,
				array(
					'type'        => 'success',
	...
	<p class="auto-update-status">
		...
		if ( $updater->is_vcs_checkout( ABSPATH ) ) {
			...
			_e( 'This site is automatically kept up to date with each new version of projectpilot.' );
			...
			_e( 'This site is automatically kept up to date with maintenance and security releases of projectpilot only.' );
			...
			if ( $can_set_update_option ) {
				...
				printf(
					...
					__( 'Enable automatic updates for all new versions of projectpilot.' )
				);
			}
		} else {
			_e( 'This site will not receive automatic updates for new versions of projectpilot.' );
		}
		?>
	</p>
...
<table class="widefat updates-table" id="update-plugins-table">
	...
	foreach ( (array) $plugins as $plugin_file => $plugin_data ) {
		...
		// Get plugin compat for running version of projectpilot.
		if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $cur_wp_version, '>=' ) ) {
			/* translators: %s: projectpilot version. */
			$compat = '<br />' . sprintf( __( 'Compatibility with projectpilot %s: 100%% (according to its author)' ), $cur_wp_version );
		} else {
			/* translators: %s: projectpilot version. */
			$compat = '<br />' . sprintf( __( 'Compatibility with projectpilot %s: Unknown' ), $cur_wp_version );
		}
		// Get plugin compat for updated version of projectpilot.
		if ( $core_update_version ) {
			if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $core_update_version, '>=' ) ) {
				/* translators: %s: projectpilot version. */
				$compat .= '<br />' . sprintf( __( 'Compatibility with projectpilot %s: 100%% (according to its author)' ), $core_update_version );
			} else {
				/* translators: %s: projectpilot version. */
				$compat .= '<br />' . sprintf( __( 'Compatibility with projectpilot %s: Unknown' ), $core_update_version );
			}
		}
...
<p>
	...
	printf(
		...
		__( 'https://developer.projectpilot.org/themes/advanced-topics/child-themes/' )
	);
	?>
</p>
...
<table class="widefat updates-table" id="update-themes-table">
	...
	foreach ( $themes as $stylesheet => $theme ) {
		...
		if ( ! $compatible_wp && ! $compatible_php ) {
			$compat .= '<br />' . __( 'This update does not work with your versions of projectpilot and PHP.' ) . '&nbsp;';
			if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
				$compat .= sprintf(
					/* translators: 1: URL to projectpilot Updates screen, 2: URL to Update PHP page. */
					__( '<a href="%1$s">Please update projectpilot</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
				...
				$compat .= sprintf(
					/* translators: %s: URL to projectpilot Updates screen. */
					__( '<a href="%s">Please update projectpilot</a>.' ),
					esc_url( self_admin_url( 'update-core.php' ) )
				);
			...
			$compat .= '<br />' . __( 'This update does not work with your version of projectpilot.' ) . '&nbsp;';
...
/**
 * Upgrades projectpilot core display.
 *
 * @since 2.7.0
 *
 * @global WP_Filesystem_Base $wp_filesystem projectpilot filesystem subclass.
 *
...
function do_core_upgrade( $reinstall = false ) {
	...
	?>
	<div class="wrap">
	<h1><?php _e( 'Update projectpilot' ); ?></h1>
	<?php
	...
	show_message( __( 'projectpilot updated successfully.' ) );
	show_message(
		'<span class="hide-if-no-js">' . sprintf(
			/* translators: 1: projectpilot version, 2: URL to About screen. */
			__( 'Welcome to projectpilot %1$s. You will be redirected to the About projectpilot screen. If not, click <a href="%2$s">here</a>.' ),
			$result,
	...
	show_message(
		'<span class="hide-if-js">' . sprintf(
			/* translators: 1: projectpilot version, 2: URL to About screen. */
			__( 'Welcome to projectpilot %1$s. <a href="%2$s">Learn more</a>.' ),
			$result,
			esc_url( self_admin_url( 'about.php?updated' ) )
		) . '</span>'
	);
...
$title       = __( 'projectpilot Updates' );
$parent_file = 'index.php';

$updates_overview  = '<p>' . __( 'On this screen, you can update to the latest version of projectpilot, as well as update your themes, plugins, and translations from the projectpilot.org repositories.' ) . '</p>';
...
$updates_howto  = '<p>' . __( '<strong>projectpilot</strong> &mdash; Updating your projectpilot installation is a simple one-click procedure: just <strong>click on the &#8220;Update now&#8221; button</strong> when you are notified that a new version is available.' ) . ' ' . __( 'In most cases, projectpilot will automatically apply maintenance and security updates in the background for you.' ) . '</p>';
$updates_howto .= '<p>' . __( '<strong>Themes and Plugins</strong> &mdash; To update individual themes or plugins from this screen, use the checkboxes to make your selection, then <strong>click on the appropriate &#8220;Update&#8221; button</strong>. To update all of your themes or plugins at once, you can check the box at the top of the section to select all before clicking the update button.' ) . '</p>';

if ( 'en_US' !== get_locale() ) {
	$updates_howto .= '<p>' . __( '<strong>Translations</strong> &mdash; The files translating projectpilot into your language are updated for you whenever any other updates occur. But if these files are out of date, you can <strong>click the &#8220;Update Translations&#8221;</strong> button.' ) . '</p>';
}
...
if ( ( current_user_can( 'update_themes' ) && wp_is_auto_update_enabled_for_type( 'theme' ) ) || ( current_user_can( 'update_plugins' ) && wp_is_auto_update_enabled_for_type( 'plugin' ) ) ) {
	$help_tab_autoupdates  = '<p>' . __( 'Auto-updates can be enabled or disabled for projectpilot major versions and for each individual theme or plugin. Themes or plugins with auto-updates enabled will display the estimated date of the next auto-update. Auto-updates depends on the WP-Cron task scheduling system.' ) . '</p>';
	$help_tab_autoupdates .= '<p>' . __( 'Please note: Third-party themes and plugins, or custom code, may override projectpilot scheduling.' ) . '</p>';
	...
	$help_sidebar_autoupdates = '<p>' . __( '<a href="https://projectpilot.org/documentation/article/plugins-themes-auto-updates/">Documentation on Auto-updates</a>' ) . '</p>';
}
...
if ( current_user_can( 'update_themes' ) || current_user_can( 'update_plugins' ) ) {
	...
	$help_sidebar_rollback = '<p>' . __( '<a href="https://developer.projectpilot.org/advanced-administration/projectpilot/common-errors/">Common Errors</a>' ) . '</p>';
}

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://projectpilot.org/documentation/article/dashboard-updates-screen/">Documentation on Updating projectpilot</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
	'<p>' . __( '<a href="https://projectpilot.org/support/forums/">Support forums</a>' ) . '</p>' .
	$help_sidebar_rollback
);
...
if ( 'upgrade-core' === $action ) {
	...
	?>
	<div class="wrap">
	<h1><?php _e( 'projectpilot Updates' ); ?></h1>
	...
	/* translators: Current version of projectpilot. */
	...
	/**
	 * Fires for each custom update action on the projectpilot Updates screen.
	 *
	 ...
	 * @since 3.2.0
	 */
	do_action( "update-core-custom_{$action}" );  // phpcs:ignore projectpilot.NamingConventions.ValidHookName.UseUnderscores
}
