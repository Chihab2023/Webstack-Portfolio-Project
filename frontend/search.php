<?php get_header(); 
    ...
    $results                    = isset($_GET['s']) ? esc_html($_GET['s']) : '';
...
<div class="container">
    ...
    <section class="middle <?php echo $post->post_name; ?> search-results">
        ...
        <?php if($user_role == 'administrator' || $user_role == 'project_manager' || $user_role == 'team_member' || $user_role == 'observer'|| $user_role == 'inspector') {
        ...
        <h2><?php printf(__('Bob the bot could not find <span>%1$s</span>.', 'wproject'), esc_html($_GET['s'])) ?></h2>
