...
define("INVALID_NONCE", __('Invalid nonce.', 'wproject'));
define("NO_PERMISSIONS", __('You do not have permission to perform this action.', 'wproject'));
...
function update_user_details() {
        ...
        /* Check the nonce */
        if (!isset($_POST['update_user_details_nonce']) || !wp_verify_nonce($_POST['update_user_details_nonce'], 'inputs')) {
            wp_send_json_error(array('message' => constant('INVALID_NONCE')));
            return;
        }

        /* Check if the current user can edit posts */
        if (!current_user_can('edit_posts')) {
            wp_send_json_error(array('message' => constant('NO_PERMISSIONS')));
            return;
        }
...
function change_theme() {
	...
	/* Check the nonce */
	if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'inputs')) {
		wp_send_json_error(array('message' => constant('INVALID_NONCE')));
		return;
	}
...
function edit_project() {
	...
	$project_manager			    = sanitize_text_field($params['project_manager']);
	$project_status				    = sanitize_text_field($params['project_status']);
	$project_creator			    = get_current_user_id(); /* a project can only be edited by its creator */
	$project_materials_total	    = sanitize_text_field($params['project_materials_total']);
	$project_contact			    = sanitize_text_field($params['project_contact']);
    $project_pep_talk_percentage    = sanitize_text_field($params['project_pep_talk_percentage']);
    $project_pep_talk_message       = sanitize_text_field($params['project_pep_talk_message']);
...
function add_new_task() {
    ...
    /* Check the nonce */
    if (!isset($_POST['add_new_task_nonce']) || !wp_verify_nonce($_POST['add_new_task_nonce'], 'inputs')) {
        wp_send_json_error(array('message' => constant('INVALID_NONCE')));
        return;
    }
	...
	$task_control		= sanitize_text_field($_POST['task_control']);
	...
	update_post_meta( $post_id, "task_name" , $task_name);
	update_post_meta( $post_id, "task_control" , $task_control);
	...
	*/
	...
	if($task_control != get_current_user_id() && $task_control != $task_owner) {
        $task_creator_info = get_userdata(get_current_user_id());
        $task_control_info = get_userdata($task_control);
        $creator_name = $task_creator_info->first_name . ' ' . $task_creator_info->last_name;
        $creator_url = get_the_permalink(109) . '/?id=' . $task_creator_info->ID;
        $message_title = sprintf( __('New Task to Control', 'wproject'));
        $task_url = get_the_permalink($post_id);
        $task_title = get_the_title($post_id);
        $message_body = sprintf( __('<a href="%1$s" style="color:#00bcd4">%2$s</a> assigned you to control a task: <a href="%3$s" style="color:#00bcd4">%4$s</a>.', 'wproject'),$creator_url, $creator_name, $task_url, $task_title);
 $subject = __('You have been assigned to control a new task', 'wproject');
        $recipient = $task_control_info->user_email;

        // Insert notification for task control user
        wp_insert_post(array (
            'post_type'         => 'message',
            'post_title'        => $message_title,
            'post_content'      => $message_body,
            'post_status'       => 'publish',
            'post_author'       => $task_control,
            'comment_status'    => 'closed',
            'ping_status'       => 'closed'
        ));

        // Send email notification to task control user
...
function edit_task() {
    ...
    /* Check the nonce */
    if (!isset($_POST['edit_task_nonce']) || !wp_verify_nonce($_POST['edit_task_nonce'], 'inputs')) { 
        wp_send_json_error(array('message' => constant('INVALID_NONCE')));
        return;
    }
...
/* function create_task_folder($task_name, $post_id) {
    $upload_dir = wp_upload_dir();
    $task_folder = $upload_dir['basedir'] . '/' . sanitize_title($task_name . '-' . $post_id);
    
    if (!file_exists($task_folder)) {
        wp_mkdir_p($task_folder);
    }
...
function custom_upload_dir($uploads) {
    global $task_folder;
    if (isset($task_folder)) {
        $uploads['path'] = $task_folder;
        $uploads['url'] = str_replace($uploads['basedir'], $uploads['baseurl'], $task_folder);
    }
    return $uploads;
} */

/* Add files */
function uploadFiles($taskName, $files) {
    // Sanitize the task name to ensure it's safe for use as a directory name
    $safeTaskName = preg_replace('/[^a-zA-Z0-9_-]/', '_', $taskName);
    
    // Create the upload directory path
    $uploadDir = "uploads/{$safeTaskName}/";
...
/* $file_url = array();
if(!empty($_FILES['task_files']['name'][0])) {
    // Assuming $post_title and $post_id are available in this scope
    global $task_folder;
    $task_folder = create_task_folder($post_title, $post_id);
...
function claim_task() {
    ...
    $no_owner_task_author	= get_current_user_id();
    // $no_owner_task_id		= sanitize_text_field('no_owner_task_id');
    // $no_owner_task_name		= sanitize_text_field('no_owner_task_name');
...
function transfer_task_ownership() {
	...
	$task_owner_id			= get_current_user_id();
