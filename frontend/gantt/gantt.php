        ...
        var wProjectChart = [
            ...
            if($task_private == 'yes') {
                if($author_id == $current_author) {
                    $task_title = str_replace(array("\r","\n"),"",get_the_title());
                    $task_link = get_the_permalink();
                } else {
                    $task_title = __('Private task', 'wpoject');
                    $task_link = '#';
                }
            } else {
                $task_title = str_replace(array("\r","\n"),"",get_the_title());
...
<script>
    $(function() {
        ...
        {
        /* Display Gantt items */
        name: "<a href='<?php echo get_category_link( $cat->term_id ) ?>'><?php echo $cat->name; ?></a>",
        desc: "",
        values: [{
            from: <?php echo $milliseconds_start; ?>,
            to: <?php echo $milliseconds_end; ?>,
            ...
            customClass: "<?php echo $class; ?>",
            dataObj: {
                title: "<?php echo $cat->name; ?><?php echo $the_avatar; ?>", 
