<?php 
    ...
    $edit_project_url           = get_the_permalink(101) . '/?project-id=' . sanitize_text_field($term_id); // Edit Project page
...
<div class="container project <?php if($print_hide_complete_tasks == 'on') { echo 'print-hide-tasks'; } ?>">
    ...
    <section class="middle">
        ...
        <?php if($project_status == 'archived' || $project_status == 'cancelled' || $project_status == 'inactive') { ?>
            <div class="project-status">
                ...
                <p><?php printf( __('This project is <strong>%1$s</strong>. To access this project, change <a href="%2$s">status</a>.', 'wproject'), esc_attr($project_status), esc_attr($edit_project_url)); ?></p>
            </div>
        <?php } else { ?>
        ...
        <?php 
            if(function_exists('gantt_pro_project')) {
                ...
                if(!wp_is_mobile()) {
                    ...
                    if($hide_gantt !='yes' && $role == 'client' && $gantt_show_dashboard == 'on' && $client_project_access == 'limited' && empty($print)) {
                        get_template_part('gantt/gantt-limited');
                    } else if($hide_gantt !='yes' && $role == 'client' && $gantt_show_dashboard == 'on' && $client_project_access == 'unlimited' && empty($print)) {
                        get_template_part('gantt/gantt');
                    } else if($hide_gantt !='yes' && $role != 'client' && $gantt_show_dashboard == 'on' && empty($print)) {
                        get_template_part('gantt/gantt');
                    }
                }
            } 
        ?>
            ...
            <div class="tabby tabby-project spacer">
                ...
                <!--/ Start My Tasks /-->
                <form class="update-task-status-form" id="update-task-status-form" method="post" data-target="update_task_status" enctype="multipart/form-data">
                            ...
                            ?>
                                <li class="priority <?php echo $overdue_class; ?> <?php echo $task_priority['slug']; ?> <?php echo $task_status['slug']; ?> <?php echo $relation; ?> <?php if($task_milestone == 'yes') { echo 'milestone'; } ?> <?php if($print_hide_user_photos) { echo 'avatars-hidden'; } ?> <?php if($task_status['slug'] == 'complete' && $minimise_complete_tasks == 'yes') { echo 'minimise'; } ?> <?php if($task_spacing) { echo 'spacing'; } ?> <?php echo str_replace(' ', '-', strtolower($context_label)); ?> <?php if($fade_on_hold && $fade_on_hold == 'on' && $task_status['slug'] == 'on-hold') { echo 'faded'; } ?> <?php if($task_timer == 'on') { echo 'time'; }?>" id="task-id-<?php echo $task_id; ?>" data-date="<?php echo strtotime($task_end_date); ?>">
                                    ...
                                    <div class="more">
                                        ...
                                        <a href="<?php echo get_the_permalink(102);?>?task-id=<?php echo esc_attr($task_id); ?>" class="edit" title="<?php _e('Edit', 'wproject'); ?>"><i data-feather="edit-3"></i></a>
                                    </div>
                ...
                $(document).ready(function() {
                    ...
                    $('.sort-my-tasks li.milestone, .sort-other-tasks li.milestone').each(function() {
                        if ($(this).hasClass('milestone') ) {
                            ...
                            var $milestone_list_item = $('<li class="milestone-'+milestone_id+'">').text(milestone_text).addClass($(this).attr('class')).removeClass(function(index, className) {
                                return (className.match(/(^|\s)(?!complete|in-progress|not-started|on-hold|incomplete|milestone)\S+/g) || []).join(' ');
                            });
                            /* Add list of milestone tasks titles into element */
                    ...
                    /* Show milestones in project sidebar as complete when task is marked complete */
                    $('.task-status-container small').click(function() {

                        var this_task_value = $(this).attr('value');
                        var this_task_data_id = $(this).attr('data');

                        var realtime_milestones_count = $('.milestones-window ul li').length;
                        var realtime_section_percentage = (100 / realtime_milestones_count);
                        
                        if(this_task_value == 'complete') {
                            $('.milestones-window ul .milestone-task-id-'+this_task_data_id).addClass('complete').prepend('<img src="<?php echo get_template_directory_uri(); ?>/images/check.svg">');

                            var realtime_current_completed = $('.milestones-window ul li.complete').length;
                            var updated_milestone_bar = (realtime_current_completed * realtime_section_percentage);

                            $('.milestones-window .milestone-bar div').css('width', updated_milestone_bar + '%');

                        } else if(this_task_value == 'incomplete' || this_task_value == 'in-progress' || this_task_value == 'on-hold' || this_task_value == 'not-started') {
                            $('.milestones-window ul .milestone-task-id-'+this_task_data_id).removeClass('complete');
                            $('.milestones-window ul .milestone-task-id-'+this_task_data_id+' img').remove();
                        ...
                        $('.milestones-window h3').text("<?php _e('Milestones', 'wproject'); ?> ("+realtime_current_completed+"/"+realtime_milestones_count+")");
                    });
    ...
    <?php /* Help topics */
        ...
        ?>
            ...
            <li><a href="<?php echo get_the_permalink(105); ?>?project-id=<?php echo esc_attr($term_id); ?>"><i data-feather="plus-circle"></i><?php _e('Add a task', 'wproject'); ?></a></li>
            <?php } ?>
            
            <?php add_to_calendar(); ?>

            <?php if($current_user_id == $project_manager_id->ID) { ?>
            <li><a href="<?php echo get_the_permalink(101); ?>?project-id=<?php echo esc_attr($term_id); ?>"><i data-feather="edit-3"></i><?php _e('Edit', 'wproject'); ?></a></li>
            
            <form class="delete-project-form" id="delete-project-form" data-target="delete_project" method="post" enctype="multipart/form-data">
    ...
    if($print && $print_hide_task_descriptions) { ?>
        <script>
            $('.more').remove();
        </script>
    <?php } 
