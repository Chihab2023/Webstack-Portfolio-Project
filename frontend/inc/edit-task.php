<?php if ( ! defined( 'ABSPATH' ) ) { exit; }
    ...
    $task_name			        = get_post_meta( $task_id, 'task_name', true );
...
<?php if($user_role == 'team_member'  || $user_role == 'inspector'|| $user_role == 'administrator' ) 
{ ?>
<form class="general-form edit-task-form" method="post" data-target="edit_task" id="edit-task-form" enctype="multipart/form-data">
<?php error_log('POST data: ' . print_r($_POST, true));?>
    <?php if($user_role == 'team_member'  ||  $user_role == 'administrator' || $user_role == 'inspector'){?>
	<fieldset>
        ...
        <li>
            ...
            <select id="localisation_generale" name="localisation_generale" required>
                <option value=""></option>
                <option value="Pont" <?php selected($localisation_generale, 'Pont'); ?>><?php _e('Pont', 'wproject'); ?></option>
                <option value="Digue_quai" <?php selected($localisation_generale, 'Digue_quai'); ?>><?php _e('Digue quai', 'wproject'); ?></option>
            </select>
        </li>
        ...
        <li>
            ...
            <select id="sub_localisation_1" name="sub_localisation_1">
                <option value=""></option>
                <option value="Pieu" <?php selected($sub_localisation_1, 'Pieu'); ?>><?php _e('Pieu', 'wproject'); ?></option>
                <option value="chevetreBA" <?php selected($sub_localisation_1, 'chevetreBA'); ?>><?php _e('chevêtre B.A', 'wproject'); ?></option>
                <option value="chevetreMetallique" <?php selected($sub_localisation_1, 'chevetreMetallique'); ?>><?php _e('chevêtre métallique', 'wproject'); ?></option>
                <option value="appareilAppui" <?php selected($sub_localisation_1, 'appareilAppui'); ?>><?php _e('Appareil d\'appui', 'wproject'); ?></option>
                <option value="digueSecondaire" <?php selected($sub_localisation_1, 'digueSecondaire'); ?>><?php _e('Digue secondaire', 'wproject'); ?></option>
                <option value="diguePrincipale" <?php selected($sub_localisation_1, 'diguePrincipale'); ?>><?php _e('Digue principale', 'wproject'); ?></option>
            </select>
        </li>
		...
		<?php } ?> 
	 <?php if($user_role == 'project_manager' || $user_role == 'inspector'|| $user_role == 'administrator' || $user_role == 'team_member' ) { ?>
	<fieldset>
	

	<legend><?php _e('ITN Process', 'wproject'); ?></legend>
		<ul>
                       
            <li>
             <label><?php _e('ITN No. Réf.', 'wproject'); ?></label><?php echo esc_html($task_name); ?>
            <input type="hidden" name="task_name" value="<?php echo esc_attr($task_name ?: 'Default Task Name'); ?>" />
           </li>
		   
           <li><label><?php _e('Date created', 'wproject'); ?>: </label><?php echo get_the_date(); ?> </li>
            ...
            <li>
                ...
                <select name="task_owner" required>
                    ...
                    <?php
                        $args = array(
                            'role__in'	=> array('team_member', 'project_manager', 'client', 'administrator', 'inspector'),
                            'orderby'	=> 'first_name',
                            'order'   	=> 'ASC'
                        );
                    ...
                    ?>
					 ...
					 <?php if($user_role == 'project_manager' || $user_role == 'inspector'|| $user_role == 'administrator' ) { ?>
					<li>
                    <label><?php _e('Select an Exterior Control', 'wproject'); ?></label>
                    <select name="task_control">
                        <option value="0"><?php _e('Nobody (Selected)', 'wproject'); ?></option>
                        <?php
                        $args = array(
                        ...
                        <option value="<?php echo esc_html( $user->ID ); ?>" <?php if($user->ID == $task_control_id) { echo 'selected'; } ?>><?php echo esc_html( $user->first_name ); ?> <?php echo esc_html( $user->last_name ); ?> - <?php echo esc_html( $user->title ); ?></option>
						 <?php } ?>
                    </select>
                </li><?php } ?>
				<li>
                <label><?php _e('Date prévu du 1er inspection', 'wproject'); ?><em class="action clear-dates"><?php _e('Clear dates', 'wproject'); ?></em></label>
                <input type="date" name="task_start_date" <?php if($task_start_date) { echo 'value="' . $task_start_date . '"'; } ?> class="merge-start" />
				</li>
                <li><label><?php _e('Date prévu du 2nd inspection', 'wproject'); ?><em class="action clear-dates"><?php _e('Clear dates', 'wproject'); ?></em></label>
    ...
    <fieldset>
        ...
        <ul>
            <li>
				...
				<?php if (!empty($_FILES['task_files']['name'][0])) {
...
<?php } ?>
    ...
    <fieldset>
        <legend><?php _e('Description', 'wproject'); ?></legend>
        <ul>
		<li>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Retrieve saved data
    const savedSubLoc2 = <?php echo json_encode($sub_localisation_2); ?>;
    const savedSubLoc3 = <?php echo json_encode($sub_localisation_3); ?>;
    ...
    function updateSubLocalisation2() {
        ...
        options.forEach(option => {
            ...
            checkbox.checked = savedSubLoc2.includes(option);
    ...
    function updateSubLocalisation3() {
        ...
        const options = subLoc3OptionsMap[value] || [];
        ...
        options.forEach(option => {
            ...
            checkbox.value = option;
            checkbox.checked = savedSubLoc3.includes(option);
            label.appendChild(checkbox);
            label.appendChild(document.createTextNode(option));
    ...
    subLocalisation2.addEventListener('change', updateSubLocalisation3);
...
?>
   ...
   <!-- <li><a href="<?php foreach ( get_the_terms( $task_id, 'project' ) as $tax ) { echo home_url() . '/project/' .  $tax->slug; } ?>"><i data-feather="folder"></i><?php _e('Go to project', 'wproject'); ?></a></li>
    ...
    <form class="delete-task-form" id="delete-task-form" data-target="delete_task" method="post" enctype="multipart/form-data">
