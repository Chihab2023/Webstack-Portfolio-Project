...
if(
    ...
    if(function_exists('gantt_pro_dashboard')) {
        ...
        if($hide_gantt !='yes' && $gantt_show_dashboard == 'on' && empty($print)) {
...
<div class="rows all-projects">
    ...
    <form class="all-project-form" method="post" data-target="delete_project" id="all-projects-form" enctype="multipart/form-data">
        ...
        <ul class="body-rows <?php if(wp_is_mobile()) { echo 'mobile'; } ?>">
            <?php 
                ...
                <li class="<?php echo $overdue_class; ?> <?php echo $project_status; ?> project-id-<?php echo $cat->term_id; ?> pm-user-id-<?php echo $pm_id; ?> <?php if($pm_id != get_current_user_id()) { echo 'not-mine'; } ?>">
                    ...
                    <?php if($pm_id == get_current_user_id() || $user_role == 'administrator') { ?>
                        <i class="delete-project" data-project-id="<?php echo $cat->term_id; ?>" title="<?php _e('Delete project', 'wproject'); ?>">
                            <i data-feather="x"></i>
                        </i>
                    <?php } ?>
                </li>
    ...
    <script>
        ...
        jQuery('.delete-project').click(function() {
            ...
            if (confirm('<?php _e('Really delete this project and all its tasks?', 'wproject'); ?>')) {
                var this_project_id = jQuery(this).data('project-id');
                jQuery('input[name="project_id"]').val(this_project_id);
                
                setTimeout(function() { 
                    jQuery('#all-projects-form').submit();
                    jQuery('.project-id-'+this_project_id).fadeOut(2000);
                }, 250);
...
function all_projects_nav() { 
    ...
    if($project_access == 'all' || $user_role == 'administrator' || $user_role == 'project_manager' || $user_role == 'inspector') { 
