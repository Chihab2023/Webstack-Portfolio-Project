...
<body id="fouc" <?php if($print) { ?>onload="window.print()"<?php } ?> class="page <?php if(is_search()) { echo ' home'; } if(is_single()) { echo $post->post_name; } if(is_singular('task')) { echo ' single'; } echo ' role-' . $user_role; ?> <?php if(is_tax()) { echo 'project-page'; } ?>">

<?php do_action( 'after_body_start' ); ?>

<!--/ Start Header /-->
<?php if(empty(esc_html($print))) { ?>
<header>
    <?php 
