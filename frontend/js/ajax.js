...
jQuery(document).ready(function($) {
	...
	$('#account-form').on('submit', function(e) {
		...
		if (window.FormData) {
			...
			formData.append('update_user_details_nonce', inputs.nonce);
			formData.append('action', 'update_user_details');
		} else {
			formData = $(this).serialize();
			formData += '&update_user_details_nonce=' + inputs.nonce;
    ...
    $('form').on('submit', function(e) {
        ...
        var the_function_name = $(this).data('target');
        //console.log(the_function_name);

        /* Don't do anything if submitting the comment form */
        if(the_function_name != 'comment_form') {

            e.preventDefault();
            disableUI();    

            if(the_function_name == 'add_new_task') {

                //console.log(the_function_name);

                var formData = new FormData(this);
                formData.append('action', 'add_new_task');
                formData.append('add_new_task_nonce', inputs.nonce);
            ...
            } else if(the_function_name == 'edit_task') { 

                var formData = new FormData(this);
                formData.append('action', 'edit_task');
                formData.append('edit_task_nonce', inputs.nonce);
	...
	function ResponseSuccess(data) {
		...
		*/
		...
		/* Project deleted */
			...
			$('.main-progress, .status-box, .gantt, .update-gantt-pro-form, .toggle-gantt-fs, .toggle-gantt-visibility, .tabby, .tips, .right .owner, .right .page-project-page, .projects-list .current, .middle h1, .project-description, .update-gantt-pro-form, .project-finished').fadeOut();
		...
		/* Start Timer */
            ...
            $('<li class="inserted"><span class="col-avatar"><img src="'+time_avatar+'" class="avatar ' + avatar_style + '" />'+time_user_name+'</span><span class="time">'+the_time+'</span><span class="col-date">'+time_date+'</span><span class="delete-time"></span></li>').insertAfter('.time-header');
