	...
	if($role == 'administrator' && !$onboarding) { ?>
		<div class="cover">
			...
			<div class="slide" id="slide-04">
				...
				<p></p>
				<form method="post" id="onboarding" data-target="complete_onboarding" enctype="multipart/form-data">
...
function favs() { 
    ...
    <li class="follow <?php if (in_array(get_the_ID(), $the_fav_tasks)) { echo 'followed'; } ?>">
        <form method="post" class="task-follow" data-target="task_follow_status" id="task-follow" enctype="multipart/form-data">
            <a>
...
?>
	<script>
		...
		$('#update-task-status-form .task-status small').click(function () {
			...
			var project_title = $('.middle h1').text();
			...
			*/
			function moveProgressBarUp() {
                ...
                <?php if(!is_front_page()) { ?>
					/* Get project ID */
				$('head title').text('<?php echo single_cat_title(); ?>' + ' (' + current_progress.toFixed(1) + '%)');  /* Update document title */
                <?php } ?>
...
function user_project_tasks_count() {
    ...
    if($user_role == 'administrator' || $user_role == 'inspector'|| $user_role == 'project_manager' || $project_access == 'all' && $user_role == 'team_member') {
        return 1;
...
//task owners and task control users
...
function messages() {
    ...
    ?>
		<div class="message-<?php echo get_the_ID(); ?> message-0<?php echo $mc++; ?>">
			<form class="mark-message-read" id="" method="post" data-target="mark_message_read" enctype="multipart/form-data">
...
function misc_output() {
	...
	$('#project_name').keyup(function () {
		$('.main-nav li ul #project-<?php if(isset($_GET['project-id'])) { echo esc_html($_GET['project-id']); } ?> em, h1').text($(this).val());
	});
	...
	<div class="main-progress">
        ...
        <script>
            setTimeout(function(){
                ...
                <?php if(!is_front_page()) { ?>
				$('head title').text('<?php echo single_cat_title(); ?>' + ' (<?php echo round($progress, 1); ?>%)');  /* Update document title */
                <?php } ?>
...
function archive_project_button() { ?>
    <form name="archive-project" id="archive-project" data-target="archive_project" method="post">
...
function delete_completed_project_tasks_button() { ?>
    <form name="delete-completed-project-tasks" id="delete-completed-project-tasks" data-target="delete_completed_project_tasks" method="post">
...
function extend_the_project() {
	...
	if($end_date && $project_status != 'archived') { 
		if($now > $due_date && $project_status !='complete' && $current_user_id == $project_manager_id->ID) { ?>
			<form class="general-form extend-project-deadline" data-target="extend_project_deadline" id="extend-project-deadline" method="post">
...
function timer_ui() { 
	...
	<div class="timer-ui">
		...
		<form class="timer-start" id="timer-start" data-target="start_timer" method="post" enctype="multipart/form-data">
		...
		<form class="timer-stop" id="timer-stop" data-target="stop_timer" method="post" enctype="multipart/form-data">
			<p class="timer">
        ...
        <form class="add-missed-time" id="add-missed-time" data-target="add_missed_time" method="post" enctype="multipart/form-data">
            <input type="number" name="missed_hrs" min="0" max="<?php echo $overtime; ?>" placeholder="Hrs" required />
            <input type="number" name="missed_mins" min="0" max="60" placeholder="Mins" required />
            <input type="date" name="missed_date" max="<?php echo date('Y-m-d'); ?>" value="<?php echo date('Y-m-d'); ?>" required />
            <input type="hidden" name="task_id" value="<?php echo $task_id; ?>" />
			<input type="hidden" name="user_id" value="<?php echo $user_id; ?>" />
			<input type="hidden" name="project_id" value="<?php echo $project_id; ?>" />
        ...
        ?>
        <div class="current-user-data" data-name="<?php echo $first_name; ?> <?php echo $last_name; ?>" data-avatar="<?php echo $small_avatar[0]; ?>" data-date="<?php echo date_i18n( get_option('date_format') ); ?>" data-avatar-style="<?php echo $the_avatar_style; ?>"></div>
...
function recent_tasks() { 
    ...
    if($show_latest_activity == 'yes') {
        ...
        if($user_role == 'project_manager' || $user_role == 'administrator' || $user_role == 'team_member'|| $user_role == 'inspector') {
...
function close_projects_selection() { ?>
    ...
    <?php 
		$task_id 	= isset($_GET['task-id']) ? esc_html($_GET['task-id']) : '';
		$project_id	= isset($_GET['project-id']) ? esc_html($_GET['project-id']) : '';
...
// Radio button for exterior control
/* function exterior_control_radio() {
    ?>
    <div class="exterior-control-radio">
        <p><?php _e('Do you want to add an exterior control?', 'wproject'); ?></p>
        <label>
            <input type="radio" name="exterior_control" value="yes" required> <?php _e('Yes', 'wproject'); ?>
        </label>
        <label>
            <input type="radio" name="exterior_control" value="no" required> <?php _e('No', 'wproject'); ?>
        </label>
    </div>
    <?php
}
add_action('before_task_assignment', 'exterior_control_radio');  */

/* Task assignee for all users */
 function task_assignee($x = -1) { 
    $wproject_settings          = wProject();
    ...
    if($user_role == 'project_manager' || $user_role == 'inspector'|| $user_role == 'administrator' || $user_role == 'team_member' && $users_can_assign_tasks || $user_role == 'client' && $client_can_assign_tasks) { 
        ?>
        <select name="task_owner" required>
            ...
            foreach ($users as $user) { 
                ?>
                <option value="<?php echo esc_attr($user->ID); ?>" <?php selected($default_task_ownership == 'yes' && $user->ID == get_current_user_id()); ?>>
                    <?php echo esc_html($user->first_name . ' ' . $user->last_name . ' - ' . $user->title); ?>
                    <?php echo $user->roles[0] == 'client' ? ' (' . esc_html($user->roles[0]) . ')' : ''; ?>
                </option>
                <?php 
            }}else{
				if($user_role == 'inspector' ) { 
            $args = array(
                'role__in'  => array('team_member'),
                'orderby'   => 'first_name',
                'order'     => 'ASC'
            );
            $users = get_users($args);
	...
	<?php
        ...
        ?>
        <input type="text" class="text" value="<?php echo esc_attr($user->first_name . ' ' . $user->last_name); ?>" disabled />
        <input type="hidden" name="task_owner" class="text" value="<?php echo esc_attr($user->ID); ?>" required />
        <?php
    }
        // Add exterior control selection if x > 0
        /* if ($x > 0) {
            ?>
            <select name="task_control" required>
                <option value=""><?php _e('Select Team Member for Control', 'wproject'); ?></option>
                <?php
...
// Process the task assignment
 ...
 /* function process_task_assignment($task_id) {
    if (isset($_POST['task_owner'])) {
        $task_owner = intval($_POST['task_owner']);
        update_post_meta($task_id, 'task_owner', $task_owner);

        if (isset($_POST['exterior_control']) && $_POST['exterior_control'] === 'yes' && isset($_POST['task_control'])) {
            $task_control = intval($_POST['task_control']);
            $control_user = get_userdata($task_control);
            
            if ($control_user && in_array('team_member', $control_user->roles)) {
...
// Hook the process_task_assignment function to the appropriate action
add_action('save_post', 'process_task_assignment');
*/ 
...
// Handle file upload
add_action('wp_ajax_handle_control_file_upload', 'handle_control_file_upload');

function handle_control_file_upload() {
    check_ajax_referer('control_file_upload', 'security');

    if (!isset($_FILES['control_file']) || !isset($_POST['post_id'])) {
        wp_send_json_error('Invalid request');
    }

    $post_id = intval($_POST['post_id']);
    $task_control_id = get_post_meta($post_id, 'task_control', true);
    $current_user = wp_get_current_user();

    if ($current_user->ID != $task_control_id) {
        wp_send_json_error('You do not have permission to upload files for this task.');
    }

    require_once(ABSPATH . 'wp-admin/includes/file.php');
    $uploadedfile = $_FILES['control_file'];
    ...
    if ($movefile && !isset($movefile['error'])) {
        $file_data = array(
            'id' => uniqid(),
            'name' => sanitize_file_name($uploadedfile['name']),
            'url' => esc_url($movefile['url'])
        );

        $control_files = get_post_meta($post_id, 'control_files', true);
        if (!is_array($control_files)) {
            $control_files = array();
        }
        $control_files[] = $file_data;
        update_post_meta($post_id, 'control_files', $control_files);

        wp_send_json_success('File uploaded successfully');
    } else {
        wp_send_json_error('There was an error uploading your file. Please try again.');
    }
}
		add_action('wp_ajax_update_subtask_comment', 'update_subtask_comment_callback');

function update_subtask_comment_callback() {
    // Check nonce for security (you should add this)
    // if (!wp_verify_nonce($_POST['nonce'], 'update_subtask_comment_nonce')) {
    ...
    // Get current subtask list
    $subtask_list = get_post_meta($task_id, 'subtask_list', true);

    if (is_array($subtask_list) && isset($subtask_list[$subtask_index])) {
        // Update the comment for the specific subtask
        $subtask_list[$subtask_index]['subtask_comment'] = $comment;

        // Save the updated subtask list
        $updated = update_post_meta($task_id, 'subtask_list', $subtask_list);

        if ($updated) {
            wp_send_json_success('Comment updated successfully');
        } else {
            wp_send_json_error('Error updating comment');
        }
    } else {
        wp_send_json_error('Invalid subtask index');
    }
}

/* // Handle file approval/rejection
function approve_control_file() {
    check_ajax_referer('control_file_action', 'nonce');
    ...
    $control_files = get_post_meta($post_id, 'control_files', true);
    if (!is_array($control_files)) {
        wp_send_json_error('No control files found.');
        return;
    }
    ...
    add_post_meta($post_id, 'control_file_log', $log_entry);
    wp_send_json_success('File approved successfully.');
}

function reject_control_file() {
    check_ajax_referer('control_file_action', 'nonce');
    if (!current_user_can('edit_posts')) {
        wp_send_json_error('You do not have permission to perform this action.');
        return;
    ...
    foreach ($control_files as &$file) {
        if ($file['id'] == $file_id) {
            ...
            $file['rejection_reason'] = $rejection_reason;
            break;
        }
    }
    update_post_meta($post_id, 'control_files', $control_files);
    ...
    add_post_meta($post_id, 'control_file_log', $log_entry);
    wp_send_json_success('File rejected successfully.');
}

add_action('wp_ajax_approve_control_file', 'approve_control_file');
add_action('wp_ajax_reject_control_file', 'reject_control_file');
...
// AJAX handler for getting projects
add_action('wp_ajax_get_projects', 'get_projects_callback');
function get_projects_callback() {
    // Replace this with your actual code to fetch projects
...
// AJAX handler for getting tasks
add_action('wp_ajax_get_tasks', 'get_tasks_callback');
function get_tasks_callback() {
    $project_id = $_GET['project_id'];
    // Replace this with your actual code to fetch tasks for the given project
    $tasks = get_posts(array('post_type' => 'task', 'numberposts' => -1, 'meta_key' => 'project_id', 'meta_value' => $project_id));
...
// AJAX handler for getting task details
add_action('wp_ajax_get_task_details', 'get_task_details_callback');
function get_task_details_callback() {
    $task_id = $_GET['task_id'];
    // Replace this with your actual code to fetch task details
    $task = get_post($task_id);
    echo '<h2>' . $task->post_title . '</h2>';
    echo '<p>' . $task->post_content . '</p>';
